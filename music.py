import os
import vlc
import datetime
import random
from apscheduler.schedulers.background import BackgroundScheduler
import time
import cv2
import customtkinter as ctk
from PIL import Image
def open_fullscreen_window(event=None):
    global n,vp,clip,emit,pf,video_playback,window_open
    video_playback=True
    if ff[-4:]==".mp4":
        func()
        clip_time = time.time()
        clip = cv2.VideoCapture(ff) 
        clip.set(cv2.CAP_PROP_POS_MSEC, emit*1000)
        cv2.namedWindow('Video Player', cv2.WINDOW_NORMAL) 
        cv2.setWindowProperty('Video Player', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
        window_open=True
        while clip.isOpened(): 
            ret, frame = clip.read()
            if ret: 
                cv2.imshow('Video Player', frame)
                key = cv2.waitKey(25) & 0xFF
                if key!=255:
                    if key == ord('f'):
                        video_playback=False
                        break
                    elif key == ord('a'):
                        rewind()
                    elif key == ord('d'):
                        fastf()
                    elif key == 32:
                        vp.pause()
                        break
                    elif key == ord('b'):
                        clip.release()
                        cv2.destroyAllWindows()
                        previous(None)
                        open_fullscreen_window(None)
                    elif key == ord('n'):
                        clip.release()
                        cv2.destroyAllWindows()
                        nextxx(None)
                        open_fullscreen_window(None)
                    elif key ==ord('e'):
                        open_equalizer_window(None)
                        break   
                    elif key == ord('r'):
                        repeat(None)
                    elif key == ord('s'):
                        shuffle()
            else:
                break
        clip.release()
        cv2.destroyAllWindows()
        window_open=False
        return video_playback,window_open
def open_new_list_window(event):
    global new_list_window, pf, bgcc, n,song_buttons,scheduler1,open_window
    if open_window==False:
        open_window=True
        new_list_window = ctk.CTkToplevel()
        new_list_window.title("Playlist")
        new_list_window.geometry("340x540+390+260")
        new_list_window.protocol("WM_DELETE_WINDOW", on_playlist_window_close)
        scrollable_frame = ctk.CTkScrollableFrame(new_list_window, width=310, height=500)
        scrollable_frame.grid(row=0, column=0, padx=1, pady=(10, 0), sticky="nsew")
        scrollable_frame.bind_all("<Button-4>", lambda e: scrollable_frame._parent_canvas.yview("scroll", -1, "units"))
        scrollable_frame.bind_all("<Button-5>", lambda e: scrollable_frame._parent_canvas.yview("scroll", 1, "units"))
        queue_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/queue.png"), size=(30, 30))
        song_buttons=[]
        for sn in range(len(pf)):
            song_namex = pf[sn][:-4]
            song_button = ctk.CTkButton(scrollable_frame, width=260, text=song_namex, font=("Arial", 20, "bold"), bg_color=bgcc, fg_color=bgcc, border_width=0, anchor="w")
            song_button.bind("<Button-1>", lambda e, index=sn: jump(index))
            song_buttons.append(song_button)
            song_button.grid(row=sn, column=0)
            queue_button = ctk.CTkButton(scrollable_frame, image=queue_icon,text="",width=1)
            queue_button.configure(command=lambda index=sn: queue(index))
            queue_button.grid(row=sn, column=1)
        scheduler1 = BackgroundScheduler()
        scheduler1.add_job(update_song_color, 'interval', seconds=1, max_instances=1,id='job1')
        scheduler1.start()
def on_playlist_window_close():
    global new_list_window,scheduler1,open_window
    scheduler1.shutdown(wait=False)
    new_list_window.destroy()
    open_window=False
def update_song_color():
    global n,song_buttons,ff
    for sn,song_button in enumerate(song_buttons):
        if sn == n:
            song_button.configure(fg_color="DarkOrchid3")
        else:
            song_button.configure(fg_color=bgcc)
def jump(index):
    global n
    if n!=index:
        vp.stop()
        n = index
        play()
def queue(index):
    global n,new_list_window,k,vp
    k=index-1
    vp.event_manager().event_detach(vlc.EventType.MediaPlayerEndReached)
    vp.event_manager().event_attach(vlc.EventType.MediaPlayerEndReached, queuenext)
    vp.event_manager().event_attach(vlc.EventType.MediaPlayerStopped, queuenext1)
def queuenext(event):
    global k,n
    n=k
    nextx(None)
def queuenext1(event):
    global vp,n,k
    n=k
    vp.stop()
def fastf():
    global vp, clip
    currtime = vp.get_time()
    newtime = currtime + 10000
    vp.pause()
    vp.set_time(newtime) 
    clip.set(cv2.CAP_PROP_POS_MSEC, newtime)
    vp.play()
def rewind():
    global vp, clip
    currtime = vp.get_time()
    newtime = currtime - 10000
    vp.pause()
    vp.set_time(newtime) 
    clip.set(cv2.CAP_PROP_POS_MSEC, newtime)
    vp.play()
def func():
        global emit
        emit=vp.get_time()//1000
        if emit==-1:
            func()
        return emit
def start():
    global song_name_label,cunt,fscreen,equalizer,song_progress_label,song_length,song_progress_slider,ptop,pf,n,vs,main,pp,video_playback,open_window,bgc,bgcc,count,song_name,playing,pbs,ews
    video_playback=False
    count=0
    theme="dark"
    if theme == "dark":
        bgc="gray14"
        bgcc="gray12"
    elif theme == "light":
        bgc="gray92"
        bgcc="gray95"
    ctk.set_appearance_mode(theme)
    ctk.set_default_color_theme("C:/Users/Tanubhav Juneja/Desktop/projects/myjson.json")
    equalizer = vlc.libvlc_audio_equalizer_new()
    cunt=0
    n = 0
    song_length=0
    open_window=False
    fscreen=False
    pfb=[]
    ptop = "C:/Users/Tanubhav Juneja/Desktop/music"
    pf = os.listdir(ptop)
    pf.sort()
    song_name = pf[n][:-4]
    playing=False
    pbs=False
    ews=False
    mainstart()
def mainstart():
    global song_name_label,cunt,equalizer,song_progress_label,song_progress_slider,song_length,ptop,pf,n,vs,main,pp,video_playback,open_window,bgc,bgcc,count,song_name,playing
    main = ctk.CTk()
    main.title("Music Player")
    main.geometry("440x250+747+370")
    main.overrideredirect(True)
    main.attributes("-alpha",0.0)
    main.lift()
    main.attributes("-alpha",100.0)
    song_name_label = ctk.CTkLabel(main, text=song_name, font=("Arial", 40, "bold"))
    song_name_label.pack(pady=10)
    song_name_label.place(relx=0.5, rely=0.15, anchor="n")
    vs=100
    vslider = ctk.CTkSlider(main, from_=0, to=100, orientation='vertical', width=20, height=90, command=vmove)
    vslider.set(vs)
    vslider.place(relx=0.97, rely=0.32, anchor="e")
    song_progress_label = ctk.CTkLabel(main, text="0:00", font=("Arial", 25, "bold"))
    song_progress_label.place(relx=0.5, rely=0.41, anchor="center")
    song_progress_slider = ctk.CTkSlider(main, from_=0, to=0, orientation='horizontal', width=410, height=20, command=lambda x: update_song_progress(song_progress_slider.get() * 1000))
    song_progress_slider.bind("<ButtonRelease-1>", lambda event: set_song_length())
    song_progress_slider.bind("<ButtonPress-1>", on_slider_press)
    song_progress_slider.bind("<ButtonRelease-1>", on_slider_release)
    song_progress_slider.place(relx=0.5, rely=0.545, anchor="center")
    npp=ctk.CTkFrame(main)
    otf=ctk.CTkFrame(main)
    playlist_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/playlist.png"), size=(30, 30))
    equalizer_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/equalizer.png"), size=(30, 30))
    shuffle_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/shuffle.png"), size=(30, 30))
    repeat_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/repeat.png"), size=(30, 30))
    pause_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/pause.png"), size=(40, 40))
    video_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/video.png"), size=(30, 30))
    previous_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/previous.png"), size=(40, 40))
    next_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/next.png"), size=(40, 40))
    minimize_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/minimize.png"), size=(20, 20))
    fullscreen_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/fullscreen.png"), size=(18, 18))
    close_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/close.png"), size=(15, 15))
    music_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/logo.png"), size=(25, 25))
    logo = ctk.CTkLabel(main, image=music_icon,width=1,text="")
    logo.place(relx=0.03,rely=0.04)
    toolbar = ctk.CTkFrame(main, bg_color="gray12",fg_color="gray12")
    minimize_button = ctk.CTkButton(toolbar, image=minimize_icon, command=sw,text="",width=1)
    minimize_button.pack(side="left", padx=2, pady=2)
    maximize_button = ctk.CTkButton(toolbar, image=fullscreen_icon, command=call_fullscreen,text="",width=1)
    maximize_button.pack(side="left", padx=2, pady=2)
    close_button = ctk.CTkButton(toolbar, image=close_icon, command=ee,text="",width=1)
    close_button.pack(side="left", padx=2, pady=2)
    toolbar.place(rely=0.01,relx=0.735)
    pre = ctk.CTkButton(npp, image=previous_icon, command=lambda: previous(None),text="",width=1)
    pp = ctk.CTkButton(npp, image=pause_icon, command=lambda: ppl(None),text="",width=1)
    next = ctk.CTkButton(npp, image=next_icon, command=lambda: nextxx(None),text="",width=1)
    pre.pack(side="left",padx=40)
    pp.pack(side="left",padx=45)
    next.pack(side="left",padx=45)
    npp.place(rely=0.6,relx=0.02)
    equalizer_button = ctk.CTkButton(otf, image=equalizer_icon, command=lambda:open_equalizer_window(None),text="",width=1)
    shuffle_button = ctk.CTkButton(otf, image=shuffle_icon, command=lambda:shuffle(None),text="",width=1)
    repeat_button = ctk.CTkButton(otf, image=repeat_icon, command=lambda:repeat(None),text="",width=1)
    fullscreen_button = ctk.CTkButton(otf, image=video_icon,command=lambda:open_fullscreen_window(None),text="",width=1)
    playlist_button = ctk.CTkButton(otf, image=playlist_icon,command=lambda:open_new_list_window(None),text="",width=1)
    playlist_button.pack(side="left",padx=20)
    fullscreen_button.pack(side="left",padx=21)
    repeat_button.pack(side="left",padx=21)
    shuffle_button.pack(side="left",padx=21)
    equalizer_button.pack(side="left",padx=21)
    otf.place(rely=0.82)
    main.bind('<Down>', lambda event: vslider.set(vslider.get()-5))
    main.bind('<Up>', lambda event: vslider.set(vslider.get()+5))
    main.bind('<Left>', previous)
    main.bind('<Alt_L>', previous)
    main.bind('<b>', previous)
    main.bind('<space>', ppl)
    main.bind('<Return>', ppl)
    main.bind('<n>', nextxx)
    main.bind('<Right>', nextxx)
    main.bind('<Alt_R>', nextxx)
    main.bind("<s>",lambda event:shuffle(None))
    main.bind("<r>",lambda event:repeat(None))
    main.bind("<e>",lambda event:open_equalizer_window(None))
    main.bind("<f>",lambda event:open_fullscreen_window(None))
    main.bind("<a>",lambda event:rewindr(None))
    main.bind("<d>",lambda event:fastff(None))
    main.bind("<p>",lambda event:open_new_list_window(None))
    main.iconbitmap(default="C:/Users/Tanubhav Juneja/Downloads/logo.ico")
    if playing != True:
        play()
        playing=True
    else:
        set_song_length()
    main.mainloop()
def pb(event):
    global main, scheduler1, pf, bgcc, n,song_buttons,pbs,scrollable_frame
    if pbs==False:
        pbs=True
        scrollable_frame = ctk.CTkScrollableFrame(main, width=400, height=620)
        scrollable_frame.place(rely=0.05)
        scrollable_frame.bind_all("<Button-4>", lambda e: scrollable_frame._parent_canvas.yview("scroll", -1, "units"))
        scrollable_frame.bind_all("<Button-5>", lambda e: scrollable_frame._parent_canvas.yview("scroll", 1, "units"))
        queue_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/queue.png"), size=(40, 40))
        song_buttons=[]
        for sn in range(len(pf)):
            song_namex = pf[sn][:-4]
            song_button = ctk.CTkButton(scrollable_frame, width=340, text=song_namex, font=("Arial", 28, "bold"), bg_color=bgcc, fg_color=bgcc, border_width=0, anchor="w")
            song_button.bind("<Button-1>", lambda e, index=sn: jump(index))
            song_buttons.append(song_button)
            song_button.grid(row=sn, column=0)
            queue_button = ctk.CTkButton(scrollable_frame, image=queue_icon,text="",width=1)
            queue_button.configure(command=lambda index=sn: queue(index))
            queue_button.grid(row=sn, column=1)
        scheduler1 = BackgroundScheduler()
        scheduler1.add_job(update_song_color, 'interval', seconds=1, max_instances=1,id='job1')
        scheduler1.start()
    else:
        scheduler1.shutdown(wait=False)
        scrollable_frame._scrollbar.destroy()
        scrollable_frame.destroy()
        pbs=False
def ew(event):
    global main,vslider,ews,equalizer_window
    if ews==False:
        ews=True
        vslider.place(relx=0.845)
        equalizer_window=ctk.CTkFrame(main)
        preamp_label = ctk.CTkLabel(equalizer_window, text="Preamp(dB)", font=("Arial", 25, "bold"))
        preamp_label.pack()
        preamp_scale = ctk.CTkSlider(equalizer_window, from_=-20, to=20, orientation='horizontal',command=update_preamp,height=25,width=280)
        preamp_scale.set(vlc.libvlc_audio_equalizer_get_preamp(equalizer))
        preamp_scale.pack()
        band_scales = []
        band_frequencies = [60, 170, 310, 600, 1000, 3000, 6000, 12000, 14000, 16000]
        for i in range(10):
            freq = band_frequencies[i]
            freq_label = ctk.CTkLabel(equalizer_window, text=f'{freq} Hz (dB)', font=("Arial", 25, "bold"))
            freq_label.pack()
            scale = ctk.CTkSlider(equalizer_window, from_=-20, to=20, orientation='horizontal',command=lambda val, i=i: update_band(i, val),height=25,width=280)
            scale.set(vlc.libvlc_audio_equalizer_get_amp_at_index(equalizer, i))
            scale.pack()
            band_scales.append(scale)
        equalizer_window.place(rely=0.05,relx=0.85)
    else:
        equalizer_window.destroy()
        ews=False
        vslider.place(relx=0.97)
def sw():
    global main,scheduler,scheduler1
    if open_window==True:
        on_playlist_window_close()
    if pbs==True:
        scheduler1.shutdown(wait=False)
    scheduler.shutdown(wait=False)
    main.destroy()
    main = ctk.CTk()
    main.geometry("40x40+1860+980")
    main.attributes('-topmost', True)
    main.overrideredirect(True)
    main.attributes("-alpha",0.0)
    main.lift()
    main.attributes("-alpha",100.0)
    fr_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/scale.png"), size=(28, 28))
    backfull = ctk.CTkButton(main, image=fr_icon,width=1,text="",command=restart)
    backfull.pack()
    main.mainloop()
def call_fullscreen():
    global main,fscreen,scheduler
    if open_window==True:
        on_playlist_window_close()
    scheduler.shutdown(wait=False)
    main.destroy()
    fscreen=True
    fullscreen()
def fullscreen():
    global ews,pbs,song_name_label,vslider,cunt,scheduler,equalizer,fscreen,song_progress_label,song_length,song_progress_slider,ptop,pf,n,vs,main,pp,video_playback,open_window,bgc,bgcc,count,song_name,playing, pf, bgcc, n,song_buttons,scheduler1
    ews=False
    pbs=False
    main = ctk.CTk()
    main.attributes('-fullscreen', True)
    main.title("Music Player")
    main.overrideredirect(True)
    main.attributes("-alpha",0.0)
    main.lift()
    main.attributes("-alpha",100.0)
    main.focus_force()
    song_name_label = ctk.CTkLabel(main, text=song_name, font=("Arial", 100, "bold"))
    song_name_label.pack(pady=10)
    song_name_label.place(relx=0.5, rely=0.15, anchor="n")
    vslider = ctk.CTkSlider(main, from_=0, to=100, orientation='vertical', width=25, height=610, command=vmove)
    vslider.set(vs)
    vslider.place(relx=0.98, rely=0.335, anchor="e")
    song_progress_label = ctk.CTkLabel(main, text="0:00", font=("Arial", 50, "bold"))
    song_progress_label.place(relx=0.5, rely=0.6, anchor="center")
    song_progress_slider = ctk.CTkSlider(main, from_=0, to=0, orientation='horizontal', width=1800, height=30, command=lambda x: update_song_progress(song_progress_slider.get() * 1000))
    song_progress_slider.bind("<ButtonRelease-1>", lambda event: set_song_length())
    song_progress_slider.bind("<ButtonPress-1>", on_slider_press)
    song_progress_slider.bind("<ButtonRelease-1>", on_slider_release)
    song_progress_slider.place(relx=0.5, rely=0.67, anchor="center")
    npp=ctk.CTkFrame(main)
    otf=ctk.CTkFrame(main)
    playlist_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/playlist.png"), size=(90,90))
    equalizer_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/equalizer.png"), size=(90,90))
    shuffle_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/shuffle.png"), size=(90,90))
    repeat_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/repeat.png"), size=(90,90))
    video_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/video.png"), size=(90,90))
    pause_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/pause.png"), size=(90,90))
    previous_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/previous.png"), size=(90,90))
    next_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/next.png"), size=(90,90))
    minimize_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/minimize.png"), size=(20, 20))
    close_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/close.png"), size=(15, 15))
    restore_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/restore.png"), size=(20, 20))
    music_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/logo.png"), size=(25, 25))
    logo = ctk.CTkLabel(main, image=music_icon,width=1,text="")
    logo.place(relx=0.01,rely=0.01)
    toolbar = ctk.CTkFrame(main, bg_color="gray12",fg_color="gray12")
    minimize_button = ctk.CTkButton(toolbar, image=minimize_icon, command=sw,text="",width=1)
    minimize_button.pack(side="left", padx=2, pady=2)
    restore_button = ctk.CTkButton(toolbar, image=restore_icon, command=restart1,text="",width=1)
    restore_button.pack(side="left", padx=2, pady=2)
    close_button = ctk.CTkButton(toolbar, image=close_icon, command=ee,text="",width=1)
    close_button.pack(side="left", padx=2, pady=2)
    toolbar.place(relx=0.94)
    pre = ctk.CTkButton(npp, image=previous_icon, command=lambda: previous(None),text="",width=1)
    pp = ctk.CTkButton(npp, image=pause_icon, command=lambda: ppl(None),text="",width=1)
    next = ctk.CTkButton(npp, image=next_icon, command=lambda: nextxx(None),text="",width=1)
    pre.pack(side="left",padx=270)
    pp.pack(side="left",padx=270)
    next.pack(side="left",padx=270)
    npp.place(rely=0.74)
    shuffle_button = ctk.CTkButton(otf, image=shuffle_icon, command=lambda:shuffle(None),text="",width=1)
    repeat_button = ctk.CTkButton(otf, image=repeat_icon, command=lambda:repeat(None),text="",width=1)
    fullscreen_button = ctk.CTkButton(otf, image=video_icon,command=lambda:open_fullscreen_window(None),text="",width=1)
    equalizer_button = ctk.CTkButton(otf, image=equalizer_icon, command=lambda:ew(None),text="",width=1)
    playlist_button = ctk.CTkButton(otf, image=playlist_icon,command=lambda:pb(None),text="",width=1)
    playlist_button.pack(side="left",padx=128)
    fullscreen_button.pack(side="left",padx=150)
    repeat_button.pack(side="left",padx=150)
    shuffle_button.pack(side="left",padx=150)
    equalizer_button.pack(side="left",padx=150)
    otf.place(rely=0.87)
    main.bind('<Down>', lambda event: vslider.set(vslider.get()-5))
    main.bind('<Up>', lambda event: vslider.set(vslider.get()+5))
    main.bind('<Left>', previous)
    main.bind('<Alt_L>', previous)
    main.bind('<b>', previous)
    main.bind('<space>', ppl)
    main.bind('<Return>', ppl)
    main.bind('<n>', nextxx)
    main.bind('<Right>', nextxx)
    main.bind('<Alt_R>', nextxx)
    main.bind("<s>",lambda event:shuffle(None))
    main.bind("<r>",lambda event:repeat(None))
    main.bind("<e>",lambda event:ew(None))
    main.bind("<f>",lambda event:open_fullscreen_window(None))
    main.bind("<a>",lambda event:rewindr(None))
    main.bind("<d>",lambda event:fastff(None))
    main.bind("<p>",lambda event:pb(None))
    set_song_length()
    main.mainloop()
def restart():
    global main,fscreen,scheduler1,scheduler
    main.destroy()
    if fscreen==True:
        fullscreen()
    if fscreen==False:
        mainstart()
def restart1():
    global main,fscreen,scheduler1,scheduler
    if pbs==True:
        scheduler1.shutdown(wait=False)
    scheduler.shutdown(wait=False)
    fscreen=False
    main.destroy()
    mainstart()
def fastff(event):
    global vp
    vp.pause()
    currtime = vp.get_time()
    newtime = currtime + 10000
    vp.set_time(newtime) 
    vp.play()
def rewindr(event):
    global vp
    vp.pause()
    currtime = vp.get_time()
    newtime = currtime - 10000
    vp.set_time(newtime)
    vp.play()
def update_song_name():
    global pf,n,song_name_label,fscreen
    song_name = pf[n][:-4]
    if  fscreen==False:
        if len(song_name)>=17:
            song_name_label.configure(text=song_name,font=("Arial", 30, "bold"))
        else:
            song_name_label.configure(text=song_name,font=("Arial", 40, "bold"))
    else:
        if len(song_name)>=17:
            song_name_label.configure(text=song_name,font=("Arial", 80, "bold"))
        else:
            song_name_label.configure(text=song_name,font=("Arial", 120, "bold"))
def ppl(event):
    global pp,pause_icon,play_icon, video_playback
    if  fscreen==False:
        play_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/play.png"), size=(40, 40))
        pause_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/pause.png"), size=(40, 40))
    else:
        play_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/play.png"), size=(90, 90))
        pause_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/pause.png"), size=(90, 90))
    status=vp.get_state()
    if status == vlc.State.Playing:
        vp.pause()
        pp.configure(image=play_icon)
    elif status ==vlc.State.Paused:
        vp.play()
        pp.configure(image=pause_icon)
        if video_playback==True:
            open_fullscreen_window(None)
def nextx(event):
    global n,pf,vp,video_playback
    n += 1
    if n >= len(pf):
        n = 0
    play()   
    if video_playback==True:
        open_fullscreen_window()
def nextxx(event):
    global n,pf,vp,video_playback
    vp.stop()
    n += 1
    if n >= len(pf):
        n = 0
    play()   
    if video_playback==True:
        open_fullscreen_window()
def previous(event):
    global n,pf,vp, video_playback
    vp.stop()
    n -= 1
    if n < 0:
        n = len(pf) - 1
    play()
    if video_playback==True:
        open_fullscreen_window()
def play():
    global n,pf,vp,ptop,ff,video_playback,pp
    vlc_args = "--no-xlib --no-video"
    vlc_instance = vlc.Instance(vlc_args.split())
    vp = vlc_instance.media_player_new()
    ff = os.path.join(ptop, pf[n])
    media = vlc_instance.media_new(ff)
    media.add_option("no-video")
    vp.set_media(media)
    update_song_name()
    vp.audio_set_volume(vs)
    vp.play()
    if  fscreen==False:
        pause_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/pause.png"), size=(40, 40))
    else:
        pause_icon = ctk.CTkImage(Image.open("C:/Users/Tanubhav Juneja/Desktop/projects/icons/pause.png"), size=(90, 90))
    pp.configure(image=pause_icon)
    set_song_length()
    vp.event_manager().event_attach(vlc.EventType.MediaPlayerEndReached, nextx)
def vmove(val):
    global vs,vp
    vs = int(val)
    vp.audio_set_volume(vs)
is_dragging = False
def on_slider_press(event):
    global is_dragging
    is_dragging = True
def on_slider_release(event):
    global is_dragging
    is_dragging = False
    on_slider_move(event)
def on_slider_move(event):
    current_time = int(song_progress_slider.get())
    minutes = current_time // 60
    seconds = current_time % 60
    timeobj = datetime.time(minute=minutes, second=seconds)
    timestr = timeobj.strftime('%M:%S')
    song_progress_label.configure(text=str(timestr)+"/"+str(time_str))
    vp.set_time(current_time * 1000)
def update_song_progress(val):
    global vp,song_length,is_dragging,current_time,song_progress_slider
    if not is_dragging:
        status = vp.get_state()
        if status == vlc.State.Playing:
            current_time = vp.get_time() // 1000
            if current_time<=0:
                current_time=0
            minutes=current_time//60
            seconds=current_time%60
            timeobj = datetime.time(minute=minutes, second=seconds)
            timestr = timeobj.strftime('%M:%S')
            song_progress_label.configure(text=str(timestr)+"/"+str(time_str))
            song_progress_slider.set(current_time)
    if video_playback==True and window_open==False:
        open_fullscreen_window(None)
def set_song_length():
    global vp,time_str,songlength,scheduler
    time.sleep(0.2)
    songlength = int(vp.get_length()) // 1000
    minutes=songlength//60
    seconds=songlength%60
    time_obj = datetime.time(minute=minutes, second=seconds)
    time_str = time_obj.strftime('%M:%S')
    song_progress_slider.configure(to=songlength)
    timestr="00:00"
    song_progress_label.configure(text=str(timestr)+"/"+str(time_str))
    status = vp.get_state()
    if status == vlc.State.Playing and video_playback==False:
        scheduler = BackgroundScheduler()
        scheduler.add_job(update_song_progress,'interval', seconds=1, args=['value'],max_instances=150,id="job2")
        scheduler.start()
def open_equalizer_window(event):
    global main
    equalizer_window = ctk.CTkToplevel(main)
    equalizer_window.title("Equalizer")
    equalizer_window.geometry("250x540+1190+260")
    preamp_label = ctk.CTkLabel(equalizer_window, text="Preamp(dB)", font=("Arial", 12, "bold"))
    preamp_label.pack()
    preamp_scale = ctk.CTkSlider(equalizer_window, from_=-20, to=20, orientation='horizontal',command=update_preamp,height=20)
    preamp_scale.set(vlc.libvlc_audio_equalizer_get_preamp(equalizer))
    preamp_scale.pack()
    band_scales = []
    band_frequencies = [60, 170, 310, 600, 1000, 3000, 6000, 12000, 14000, 16000]
    for i in range(10):
        freq = band_frequencies[i]
        freq_label = ctk.CTkLabel(equalizer_window, text=f'{freq} Hz (dB)', font=("Arial", 12, "bold"))
        freq_label.pack()
        scale = ctk.CTkSlider(equalizer_window, from_=-20, to=20, orientation='horizontal',command=lambda val, i=i: update_band(i, val),height=20)
        scale.set(vlc.libvlc_audio_equalizer_get_amp_at_index(equalizer, i))
        scale.pack()
        band_scales.append(scale)
def update_preamp(val):
    vlc.libvlc_audio_equalizer_set_preamp(equalizer, float(val))
    vp.set_equalizer(equalizer)
def update_band(index, val):
    vlc.libvlc_audio_equalizer_set_amp_at_index(equalizer, float(val), index)
    vp.set_equalizer(equalizer)
def shuffle(event):
    global pf,cunt
    if cunt%2!=0:
        repeat(None)
    random.shuffle(pf)
    return pf
def ee():
    global vp,main
    vp.stop()
    main.destroy()
    os._exit(0)
def repeat(event):
    global cunt,pf,pfb,csg
    cunt+=1
    if cunt%2!=0:
        csg=pf[n]
        pfb=pf.copy()
        pf=[]
        pf.append(csg)
    elif cunt%2==0:
        pf=[]
        pf=pfb.copy()
    return pf
start()